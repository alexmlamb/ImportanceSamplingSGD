...
class LogisticRegression(object):
    ...
    def negative_log_likelihood(self, y):
        ...
        return -1.0 * T.log(self.p_y_given_x)[T.arange(y.shape[0]), y]
# end-snippet-2
...
def load_data(dataset, upsample_zero):
    ...
    #if upsampling zeros, modify train_set to do upsampling
    if upsample_zero: 
        x = train_set[0]
        y = train_set[1]
        ...
        for i in range(0, len(x)): 
            if y[i] == 0: 
                new_x += [x[i]] * upsample_rate
                new_y += [y[i]] * upsample_rate
            else:
                new_x += [x[i]]
                new_y += [y[i]]
