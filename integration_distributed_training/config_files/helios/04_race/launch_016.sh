...
OUTPUT=`msub ~/Documents/ImportanceSamplingSGD/integration_distributed_training/config_files/helios/04_race/launch_016.sh`
OUTPUT=`echo $OUTPUT | tr -d " "`
echo $OUTPUT

for i in `seq 2 24`;
do
    OUTPUT=`msub -l depend=${OUTPUT} ~/Documents/ImportanceSamplingSGD/integration_distributed_training/config_files/helios/04_race/launch_016.sh`
    OUTPUT=`echo $OUTPUT | tr -d " "`
    echo $OUTPUT
done
